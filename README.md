...
**Question 表新增了 `title` 字段，并修复了权限逻辑。拉取代码后请执行：**

```powershell
# 1. 拉取最新代码
git pull origin main

# 2. 执行数据库迁移（新增 title 字段）
python manage.py migrate

# 3. 删除旧数据并重新导入题目（让 title 生效）
python manage.py shell -c "from apps.questions.models import Question; Question.objects.all().delete()"
python manage.py import_exercises --teacher-id=1
```

> ⚠️ 如果跳过第 3 步，旧题目会缺少 `title`，前端可能显示异常。
